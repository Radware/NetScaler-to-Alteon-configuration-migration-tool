...
def is_add_server_fqdn(line):
    ...
    if split_line[0] == "add" and split_line[1] == "server":
        third_param = split_line[3].strip()
        # Check if third parameter is a domain name AND NOT an IP address
        # If it's followed by an IP address, it's an A record, not FQDN
        if is_fqdn(third_param):
            # Check if there's an IP address after the domain name
            if len(split_line) > 4:
                # Check if next parameter looks like an IP address
                potential_ip = split_line[4].strip()
                if is_valid_ip(potential_ip):
                    return False  # This is an A record (domain + IP), not FQDN
            return True  # This is a true FQDN server
    return False
...
service_mapping_to_ALT = [
    ...
    {"service" : "SMTP", "protocol":"TCP", "forceproxy":"True", "Supported": "True", "Application": "SMTP"},
    {"service" : "POP3", "protocol":"TCP", "forceproxy":"True", "Supported": "True", "Application": "POP3"},
...
# Port-based application mapping for reserved ports on Alteon
# This overrides the service type mapping when specific ports are detected
# Only uses applications that exist in Alteon's supported list
port_based_application_mapping = {
    25: "SMTP",        # SMTP - exists in Alteon
    110: "POP3",       # POP3 - exists in Alteon
    143: "imap",       # IMAP - use proper imap application
    389: "LDAP",       # LDAP - exists in Alteon
    587: "Basic-SLB",  # SMTP Submission (use Basic-SLB)
    993: "SSL",        # IMAPS - use SSL app for secure IMAP
    995: "Basic-SLB",  # POP3S (use Basic-SLB)
    # Note: Port 135 (RPC) and other ports will use default Basic-SLB
}

def get_application_by_port(port):
    """
    Get the appropriate Alteon application for a given port number.
    Returns the application name if the port has a specific mapping, None otherwise.
    """
    try:
        port_num = int(port)
        return port_based_application_mapping.get(port_num)
