...
class Netscaler:
    ...
    def handle_add_lb_vserver(self, line):
        ...
        # Handle flags starting from index 7
        i = 7
            ...
            if item.startswith("-"):
                ...
                for add_lb_vserver_flag in add_lb_vserver_flags:
                    if add_lb_vserver_flag['netscaler_vserver_feature'] == flag:
                        # Handle comment which might have quotes and multiple words
                        if flag == "comment" and i + 1 < len(split_line):
                            # Join all remaining words for comment, handling quotes
                            comment_parts = []
                            j = i + 1
                            if split_line[j].startswith('"'):
                                # Handle quoted comment
                                while j < len(split_line):
                                    comment_parts.append(split_line[j])
                                    if split_line[j].endswith('"'):
                                        break
                                    j += 1
                                # Join and clean the comment - remove quotes and any trailing newlines
                                flag_value = " ".join(comment_parts).strip('"').strip('\n').strip()
                                i = j  # Skip processed comment words
                            else:
                                flag_value = split_line[j].strip('\n').strip()
                            add_lb_vserver_virt_dict[flag] = flag_value
                            break
                        elif i + 1 < len(split_line):
                            flag_value = split_line[i + 1].strip()
                            add_lb_vserver_virt_dict[flag] = flag_value
                        break
            i += 1
    ...
    def handle_add_server(self, line):
        ...
        i = 4  # Start from index 4 (after server name and IP)
        while i < len(split_line):
            item = split_line[i]
            if item.startswith("-"):
                flag = item  # Keep the flag with the "-" prefix
                    ...
                    if add_server_flag == flag:
                        ...
                        if flag == "-comment" and i + 1 < len(split_line):
                            ...
                            if split_line[j].startswith('"'):
                                ...
                                flag_value = split_line[j].strip('\n').strip()
                            add_server_dict[flag.strip("-")] = flag_value
                            break
                        elif i + 1 < len(split_line):
                            flag_value = split_line[i + 1].strip()
                            add_server_dict[flag.strip("-")] = flag_value
    ...
    def handle_add_server_fqdn(self, line):
        ...
        i = 4  # Start from index 4 (after server name and fqdn)
        while i < len(split_line):
            item = split_line[i]
            if item.startswith("-"):
                flag = item.strip("-")
                ...
                if i + 1 < len(split_line):
                    ...
                    # Flag without value, set to empty string or True
                    add_server_fqdn_dict[flag] = True
            i += 1
        ...
        for index, item in enumerate(split_line):
            if index > 3 and item.startswith("-"):
                ...
                if index + 1 < len(split_line):
                    ...
                    bind_ssl_vserver_dict[flag] = None  # Or some default value
