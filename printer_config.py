...
def print_real_server(real_alteon_object):
    ...
    # Clean any existing quotes and newlines from comment to avoid double quoting
    if comment:
    ...
    if len(add_ports_lst) > 0 and comment:
        line = f'/c/slb/real {name}/{state}/ipver {ip_version}/rip {ip_address}/name "{comment}"/addport {add_ports_lst[0]}'
    if len(add_ports_lst) > 0 and not comment:
        line = f'/c/slb/real {name}/{state}/ipver {ip_version}/rip {ip_address}/addport {add_ports_lst[0]}'
    return line
...
def print_virt(virt_alteon_object):
    ...
    for attr, value in attributes.items():
        if value and str(value).strip():  # Check if value is not None, not empty, and not just spaces
            if attr == "vname":
                # Strip existing quotes and add proper quotes
                clean_value = str(value).strip('"')
                config_lines.append(f"    {attr} \"{clean_value}\"")
            else:
                config_lines.append(f"    {attr} {value}")
